...
async function carregarCards(qtd) {
    const dados = new URLSearchParams();
    dados.append('carregaX', qtd);
    ...
    try {
        ...
        if (data.success) {
            ...
            // Renderiza APENAS OS PRIMEIROS 2
            const carrega2 = data.data.slice(0, 2);
            renderizarCards(carrega2);
        }
    } catch (err) { }
}
carregarCards(localStorage.getItem('qtdTotal'));
            ...
            <div class="componente" style="position:relative">
                ...
                <div class="componente-info">
                    ...
                    <div class="componente-opcoes">
                        ${acoesHTML}
                        <button id="${favoritoId}" class="componente-btn" onclick="favoritar(this)" title="Favoritar"></button>
                    </div>
                </div>
            </div>
        `;
